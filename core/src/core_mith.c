...
int bmark_verify_core(void *in_params) {
	...
	int data_id;
	e_u16 crc[4];
	e_u16 seedcrc=crc16_cm(p->results->seed1,0);
	seedcrc=crc16_cm(p->results->seed2,seedcrc);
	seedcrc=crc16_cm(p->results->seed3,seedcrc);
	seedcrc=crc16_cm(p->size,seedcrc);
